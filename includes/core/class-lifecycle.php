...
class Lifecycle {
	...
	use \CodeSoup\ACFAdminCategories\Traits\LoggingTrait;
	 ...
	 * @since 1.0.0
	 */
	private const VERSION_OPTION = 'codesoup_aac_version';
	 ...
	 * @since 1.0.0
	 */
	private const OPTIONS_KEY = 'codesoup_aac_options';
	...
	public function activate(): void {
		try {
			...
			update_option( 'codesoup_aac_activated_time', time() );
			...
			wp_die(
				...
				esc_html__( 'Plugin Activation Error', 'codesoup_aac' ),
	...
	public function deactivate(): void {
		try {
			...
			update_option( 'codesoup_aac_deactivated_time', time() );
	...
	public function uninstall(): void {
		try {
			...
			delete_option( 'codesoup_aac_activated_time' );
			delete_option( 'codesoup_aac_deactivated_time' );
	...
	private function clear_caches(): void {
		...
		delete_transient( 'codesoup_aac_assets_manifest' );
	...
	private function clear_plugin_transients(): void {
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
				'_transient_timeout_codesoup_aac_%'
			)
		);
	}
}
