...
class ACFAdminCategories_Autoloader {
	...
	private function setup_namespace_mappings(): void {
		...
		$this->namespace_map = array(
			'CodeSoup\\ACFAdminCategories\\Core\\'       => $plugin_dir . '/includes/core',
			'CodeSoup\\ACFAdminCategories\\Admin\\'      => $plugin_dir . '/includes/admin',
			'CodeSoup\\ACFAdminCategories\\Frontend\\'   => $plugin_dir . '/includes/frontend',
			'CodeSoup\\ACFAdminCategories\\Providers\\'  => $plugin_dir . '/includes/providers',
			'CodeSoup\\ACFAdminCategories\\Abstracts\\'  => $plugin_dir . '/includes/abstracts',
			'CodeSoup\\ACFAdminCategories\\Interfaces\\' => $plugin_dir . '/includes/interfaces',
			'CodeSoup\\ACFAdminCategories\\Traits\\'     => $plugin_dir . '/includes/traits',
			'CodeSoup\\ACFAdminCategories\\'             => $plugin_dir . '/includes',
		);
	}
	...
	private function setup_transformations(): void {
		$this->transformations = array(
			...
			// Underscore-based class naming convention: class-acf-setup.php (with class- prefix)
			'underscore' => function ( string $class_name ): string {
				if ( strpos( $class_name, '_' ) !== false ) {
					// Convert underscores to hyphens and make lowercase, with class- prefix
					$kebab_case = strtolower( str_replace( '_', '-', $class_name ) );
					return 'class-' . $kebab_case . '.php';
				}
				return $this->transformations['wordpress']( $class_name );
			},
	...
	private function transform_class_name_to_file_name( string $class_name ): string {
		...
		// Check for underscore-based naming (like ACF_Setup)
		if ( strpos( $class_name, '_' ) !== false ) {
			return $this->transformations['underscore']( $class_name );
		}
...
if ( ! isset( $GLOBALS['codesoup_aac_autoloader'] ) ) {
	...
	$GLOBALS['codesoup_aac_autoloader'] = $codesoup_aac_autoloader;
}
